...
public class ProjectSettingConfig implements SearchableConfigurable {
    ...
    @Override
    public void apply() {
        LogicUtils.getLogic().conOrEnd(mainGui.getRemBaseValue().getText(), text -> Objects.nonNull(text) && NumberUtils.isNumber(mainGui.getRemBaseValue().getText()), text -> constValue.setRemBaseValue(mainGui.getRemBaseValue().getText()));
